package ceneax.lib.ccv.widget.camera;
...
public class CameraHelper implements Camera.PreviewCallback {
    ...
    public void onPreviewFrame(byte[] nv21, Camera camera) {
        if (cameraListener != null) {
            ...
            if (mCameraId == 1) {
                // 前置
                data = CVUtil.rotateYUVDegree270AndMirror(nv21, previewSize.width, previewSize.height);
            } else {
                // 后置
                data = CVUtil.rotateYUV420Degree90(nv21, previewSize.width, previewSize.height);
            }

            // 将图像转为bitmap
            bitmap = CVUtil.nv21ToBitmap(data, previewSize.height, previewSize.width);

            cameraListener.onPreview(data, camera, bitmap);
        }
    }
